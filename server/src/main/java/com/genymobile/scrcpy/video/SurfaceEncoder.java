...
public class SurfaceEncoder implements AsyncProcessor {
        ...
        do {
            ...
            try {
                ...
                if (outputBufferId >= 0 && bufferInfo.size > 0) {
                    ...
                    if (isConfig || controller == null || !controller.isVideoPaused()) {
    ...
    public void requestKeyFrame() {
        MediaCodec runningCodec = reset.getRunningMediaCodec();
        if (runningCodec != null) {
            try {
                android.os.Bundle b = new android.os.Bundle();
                b.putInt(MediaCodec.PARAMETER_KEY_REQUEST_SYNC_FRAME, 0);
                runningCodec.setParameters(b);
                Ln.d("Requested video keyframe");
            } catch (IllegalStateException e) {
                Ln.w("Could not request keyframe, codec not in executing state", e);
            }
        }
    }
    ...
    public void start(TerminationListener listener) {
        thread = new Thread(() -> {
            ...
            try {
                ...
                if (controller != null && !firstFrameSent) {
                    controller.sendMessage(com.genymobile.scrcpy.control.DeviceMessage.createError(1, "Configuration error: " + e.getMessage()));
                }
            } catch (IOException e) {
                ...
                if (!IO.isBrokenPipe(e)) {
                    ...
                    if (controller != null && !firstFrameSent) {
                        controller.sendMessage(com.genymobile.scrcpy.control.DeviceMessage.createError(1, "Video encoding error: " + e.getMessage()));
                    }
                }
            } catch (Exception e) {
                Ln.e("Exception during capture/encoding", e);
                if (controller != null && !firstFrameSent) {
                    controller.sendMessage(com.genymobile.scrcpy.control.DeviceMessage.createError(1, "Capture failed: " + e.getMessage()));
                }
            } finally {
